...
<template>
  <PageLayout navTitle="请假单"  backRouteName="index" routeMethod="pushTab">
    <view class="wrap">
      <z-paging
          ref="paging"
          :fixed="false"
...
import {columns} from './EduQingjiadanData';
defineOptions({
  name: 'EduQingjiadanList',
  options: {
    styleIsolation: 'shared',
  }
})
//分页加载配置
let { toast, router, paging, dataList, queryList } = usePageList('/edu/eduQingjiadan/list');
...
const handleAction = (val, item) => {
  if (val == 'del') {
    http.delete("/edu/eduQingjiadan/delete?id="+item.id,{id:item.id}).then((res) => {
      toast.success('删除成功~')
      paging.value.reload()
    })
  }
