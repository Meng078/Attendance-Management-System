      ...
      <view class="form-container">
        <wd-form ref="form" :model="myFormData">
          ...
          <view class="{ 'mt-14px': 0 == 0 }">
              <wd-input
                  label-width="100px"
                  v-model="myFormData['days']"
                  :label="get4Label('请假天数')"
                  name='days'
                  prop='days'
                  placeholder="请选择请假天数"
                  :rules="[
...
const dataId = ref('')
const backRouteName = ref('EduQingjiadanList')
// 定义 initForm 方法
const initForm = (item) => {
...
// 初始化数据
const initData = () => {
  http.get("/edu/eduQingjiadan/queryById",{id:dataId.value}).then((res) => {
    if (res.success) {
      let obj = res.result
...
const handleSubmit = async () => {
 // 判断字段必填和正则
  ...
  let url = dataId.value?'/edu/eduQingjiadan/edit':'/edu/eduQingjiadan/add';
  form.value
    .validate()
    .then(({ valid, errors }) => {
      if (valid) {
